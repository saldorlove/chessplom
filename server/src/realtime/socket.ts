...
import {
  applyFriendClockMove,
  clearFriendDrawOffer,
  clearFriendRematchOffer,
  createFriendRoom,
  createRematchFriendRoom,
  deleteFriendRoom,
  findActiveFriendRoomForPlayer,
  getExpectedMoveSide,
  getFriendClockSnapshot,
  getFriendRoom,
  getFriendRoomOpponentBySide,
  getFriendRoomPlayer,
  getFriendRoomPlayerBySide,
  joinFriendRoom,
  markFriendRoomEnded,
  markSocketDisconnected,
  reconnectFriendRoomPlayer,
  registerFriendRoomMove,
  setFriendDrawOffer,
  setFriendRematchOffer,
  stopFriendClock,
  type FriendColorChoice,
  type FriendRoomMode,
  type FriendRoomMove,
  type FriendSide,
} from "./friendRooms.js";

type CreateFriendRoomPayload = {
  userId?: string | null;
  username?: string;
...
type FriendRoomActionPayload = {
  roomCode?: string;
  side?: FriendSide;
};

type FriendClientGameEndedPayload = {
  roomCode?: string;
  side?: FriendSide;
};
...
type FriendInviteDeclinePayload = {
  inviteId?: string;
  roomCode?: string;
};
...
function getSafeUsername(value: unknown) {
...
function getSafeColor(value: unknown): FriendColorChoice {
...
function sideToColor(side: FriendSide): "white" | "black" {
...
function getPreferredSide(color: FriendColorChoice): FriendSide | null {
...
async function areUsersFriends(firstUserId: string, secondUserId: string) {
...
function chooseOnlineSides(
  firstColor: FriendColorChoice,
  secondColor: FriendColorChoice
): OnlineSidePair | null {
...
function isValidMovePayload(move: unknown): move is FriendRoomMove {
  if (!move || typeof move !== "object") {
    return false;
  }

  const maybeMove = move as Partial<FriendRoomMove>;

  return (
    typeof maybeMove.from === "string" &&
    typeof maybeMove.to === "string" &&
    typeof maybeMove.moveIndex === "number"
  );
}

function getActionContext(payload: FriendRoomActionPayload, socketId: string) {
...
function buildPlayUrlForRoomPlayer({
  room,
  playerSide,
  mode,
}: {
  room: NonNullable<ReturnType<typeof getFriendRoom>>;
  playerSide: FriendSide;
  mode: "friend" | "online";
}) {
...
function buildActiveGamePayload({
  room,
  playerSide,
}: {
  room: NonNullable<ReturnType<typeof getFriendRoom>>;
...
export function setupSocketServer({
  ...
  const onlineQueue = new Map<string, OnlineQueueEntry>();
  const userSockets = new Map<string, Set<string>>();
  const socketUserIds = new Map<string, string>();
  const pendingFriendInvites = new Map<string, PendingFriendInvite>();

  function registerSocketUser(socketId: string, userId: string | null) {
  ...
  function removeSocketFromUserMaps(socketId: string) {
  ...
  function getOnlineSocketsForUser(userId: string) {
    ...
    return Array.from(socketIds).filter((socketId) =>
      Boolean(io.sockets.sockets.get(socketId))
    );
  }
  ...
  io.on("connection", (socket) => {
    socket.on(
      ...
      (payload: FriendPresencePayload, callback) => {
    ...
    socket.on(
      "friend-presence:get",
      (payload: FriendPresenceGetPayload, callback) => {
    ...
    socket.on(
      ...
      async (payload: FriendInvitePayload, callback) => {
        try {
    ...
    socket.on(
      ...
      (payload: FriendInviteDeclinePayload, callback) => {
        const inviteId = payload?.inviteId;
        const roomCode = payload?.roomCode?.trim().toUpperCase();

        const invite =
          (inviteId ? pendingFriendInvites.get(inviteId) : undefined) ??
          Array.from(pendingFriendInvites.values()).find(
    ...
    socket.on("active-game:get", (payload: ActiveGamePayload, callback) => {
      const requestedMode =
        payload?.mode === "friend" || payload?.mode === "online"
          ? payload.mode
          : undefined;
    ...
    socket.on(
      "online:find-game",
      (payload: OnlineFindGamePayload, callback) => {
        const entry: OnlineQueueEntry = {
          socketId: socket.id,
          userId: getSafeUserId(payload?.userId),
          username: getSafeUsername(payload?.username),
          avatarUrl: payload?.avatarUrl ?? null,
    ...
    socket.on("friend:join-room", (payload: JoinFriendRoomPayload, callback) => {
      const roomCode = payload?.roomCode?.trim().toUpperCase();
    ...
    socket.on("friend:join-play", (payload: JoinFriendPlayPayload, callback) => {
      const roomCode = payload?.roomCode?.trim().toUpperCase();
    ...
    socket.on("friend:move", (payload: FriendMovePayload, callback) => {
    ...
    socket.on(
      "friend:client-game-ended",
      (payload: FriendClientGameEndedPayload, callback) => {
        const context = getActionContext(payload, socket.id);
    ...
    socket.on("friend:resign", (payload: FriendRoomActionPayload, callback) => {
      const context = getActionContext(payload, socket.id);
    ...
    socket.on(
      "friend:draw-offer",
      (payload: FriendRoomActionPayload, callback) => {
        const context = getActionContext(payload, socket.id);
    ...
    socket.on(
      ...
      (payload: FriendRoomActionPayload, callback) => {
        ...
        const offerPlayer = getFriendRoomPlayerBySide(
          room,
          room.rematchOfferFrom
        );
        ...
        const hostOpponent = getFriendRoomOpponentBySide(
          rematchRoom,
          rematchRoom.host.side
        );
        const hostPlayUrl = buildPlayUrlForRoomPlayer({
          room: rematchRoom,
        ...
        const guestOpponent = getFriendRoomOpponentBySide(
          rematchRoom,
          rematchRoom.guest.side
        );
        const guestPlayUrl = buildPlayUrlForRoomPlayer({
          room: rematchRoom,
    ...
    socket.on("disconnect", () => {
      ...
      const disconnectEvents = markSocketDisconnected(
        socket.id,
        (room, disconnectedSide) => {
          stopFriendClock(room);
          markFriendRoomEnded(room);
