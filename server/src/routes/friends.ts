...
function toFriendUser(user: FriendUserSource) {
  return {
    id: user.id,
...
async function deleteFriendship(firstUserId: string, secondUserId: string) {
  const [userAId, userBId] = getFriendshipPair(firstUserId, secondUserId);
...
async function getPendingRequestsForUsers(userId: string, targetUserIds: string[]) {
...
router.get("/", requireAuth, async (req, res) => {
  try {
    ...
    const friends = friendships.map((friendship) =>
      toFriendUser(friendship.userAId === userId ? friendship.userB : friendship.userA)
    );
