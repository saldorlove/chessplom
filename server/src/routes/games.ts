...
function getUsernameKey(value: string) {
  return value.trim().toLowerCase();
}

function getOwnerSide({
  username,
  whiteName,
  blackName,
}: {
...
function getRatingDeltaForResult({
  result,
  ownerSide,
}: {
  result: string;
...
function getDateQueryValue(value: unknown, endOfDay = false) {
  const raw = getSingleQueryValue(value);
...
function normalizeMoveTimes(value: unknown) {
  if (!Array.isArray(value)) {
    return [];
  }
...
function getDurationMs(moveTimesMs: number[]) {
  return Math.round(
    moveTimesMs.reduce((sum, value) => {
...
function getTimeControlCategory(timeControl: string) {
...
function normalizeGameSource(value: unknown) {
...
function getResultFilter(resultGroup?: string) {
  if (resultGroup === "white-win") return "1-0";
  if (resultGroup === "black-win") return "0-1";
  if (resultGroup === "draw") return "1/2-1/2";

  return undefined;
}

function getOrderBy(sort: SortValue): Prisma.GameOrderByWithRelationInput {
  if (sort === "oldest") {
...
router.get("/", async (req, res) => {
  try {
    ...
    const timeControlCategory = getSingleQueryValue(
      req.query.timeControlCategory
    );
...
router.post("/", async (req, res) => {
  try {
    ...
    const normalizedResultReason =
      typeof resultReason === "string" && resultReason.trim()
        ? resultReason.trim()
        : null;
    ...
    const shouldUpdateRating =
      normalizedSource === "online-play" && Boolean(owner && ownerSide);
    ...
    const ratingAfter =
      ratingBefore !== null && ratingDelta !== null
