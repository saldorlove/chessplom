...
type OpeningMatchKind = "line" | "position" | "family";

type OpeningFamilyPattern = {
  slug: string;
  id: string;
  title: string;
  subtitle: string;
...
const OPENING_FAMILY_PATTERNS: OpeningFamilyPattern[] = [
  {
...
function getFenKey(fen: string) {
  return fen.split(" ").slice(0, 4).join(" ");
}

function buildPositionKeys(moves: string[]) {
...
function findFamilyPatternMatch(
  ...
  for (const pattern of OPENING_FAMILY_PATTERNS) {
    ...
    const matchedRequiredCount = pattern.required.filter((move) =>
      includesNormalizedMove(patternMoves, move)
    ).length;

    const minRequired =
      pattern.minRequiredMatches ?? Math.max(pattern.required.length, 1);

    if (
      matchedRequiredCount < minRequired ||
      matchedRequiredCount < MIN_FAMILY_SIGNATURE_MATCH_COUNT
    ) {
      continue;
    }

    const matchedOptionalCount = (pattern.optional ?? []).filter((move) =>
      includesNormalizedMove(patternMoves, move)
    ).length;
...
function asArrowArray(value: unknown): OpeningArrowDto[] {
  if (!Array.isArray(value)) {
    return [];
  }
...
function mapOpeningDetail(opening: {
  ...
  return {
    ...
    variations: opening.variations.map((variation) => {
      const steps = [...variation.steps].sort(
        (left, right) => left.moveIndex - right.moveIndex
      );

      const annotations = steps.reduce<Record<number, {
        explanation: string;
        arrows: OpeningArrowDto[];
        squares: string[];
      }>>((acc, step) => {
        acc[step.moveIndex] = {
...
router.post("/identify", async (req, res) => {
  try {
    const moves = Array.isArray(req.body?.moves)
      ? req.body.moves
          .map((move: unknown) => (typeof move === "string" ? move.trim() : ""))
          .filter(Boolean)
      : [];
    ...
    let bestMatch:
      | {
          ...
          variation: (typeof openings)[number]["variations"][number];
          matchedPlyCount: number;
          totalPlyCount: number;
          line: string[];
          matchType: OpeningMatchKind;
        }
      | null = null;
    ...
    for (const opening of openings) {
      for (const variation of opening.variations) {
        ...
        for (
          let index = 0;
          index < Math.min(moves.length, steps.length);
          index += 1
        ) {
    ...
    if (!hasReliableLineMatch) {
      ...
      for (const opening of openings) {
        for (const variation of opening.variations) {
          ...
          if (
            matchedPlyCount >= MIN_POSITION_PLY_FOR_OPENING_IDENTIFICATION &&
            (!bestMatch ||
              matchedPlyCount > bestMatch.matchedPlyCount ||
              (matchedPlyCount === bestMatch.matchedPlyCount &&
                steps.length > bestMatch.totalPlyCount))
    ...
    const hasReliableDetailedMatch =
      Boolean(bestMatch) &&
      bestMatch!.matchedPlyCount >=
        MIN_MATCHED_PLY_FOR_OPENING_IDENTIFICATION;
    ...
    if (!hasReliableDetailedMatch) {
      ...
      if (familyMatch) {
        const opening = openings.find(
          (item) => item.slug === familyMatch.pattern.slug
        );
        ...
        if (opening) {
          res.json({
            ...
            matchedPlyCount:
              familyMatch.matchedRequiredCount + familyMatch.matchedOptionalCount,
            totalPlyCount:
              familyMatch.pattern.required.length +
              (familyMatch.pattern.optional?.length ?? 0),
            confidence: Math.min(
              0.85,
              (familyMatch.matchedRequiredCount +
    ...
    res.json({
      ...
      confidence: Math.min(
        1,
        bestMatch.matchedPlyCount / Math.max(4, bestMatch.totalPlyCount)
      ),
