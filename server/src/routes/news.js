...
import { getPublishedNewsArticleBySlug, getPublishedNewsArticles, saveNewsArticles, } from "../services/newsStorage.js";
const router = Router();
...
router.get("/", async (_req, res) => {
    try {
        ...
        res.status(502).json({
            message: "Не удалось получить шахматные новости. Попробуйте обновить страницу позже.",
        });
    }
});
router.post("/refresh", async (_req, res) => {
    try {
        ...
        res.status(502).json({
            message: "Не удалось обновить шахматные новости из внешнего источника.",
        });
    }
});
router.get("/:slug", async (req, res) => {
    try {
        ...
        res.status(502).json({
            message: "Не удалось получить шахматную новость. Попробуйте обновить страницу позже.",
        });
    }
});
