...
const EMAIL_CODE_RESEND_COOLDOWN_SECONDS = getNumberEnv(
...
const PASSWORD_RESET_CODE_TTL_MINUTES = getNumberEnv(
  "PASSWORD_RESET_CODE_TTL_MINUTES",
  EMAIL_CODE_TTL_MINUTES,
  1,
  60
);
const PASSWORD_RESET_CODE_RESEND_COOLDOWN_SECONDS = getNumberEnv(
  "PASSWORD_RESET_CODE_RESEND_COOLDOWN_SECONDS",
  EMAIL_CODE_RESEND_COOLDOWN_SECONDS,
  10,
  3600
);
const PASSWORD_RESET_CODE_MAX_ATTEMPTS = getNumberEnv(
...
function hashVerificationCode(emailCanonical: string, code: string) {
  return createHash("sha256")
    .update(`${emailCanonical}:${code}:${getEmailCodeSecret()}`)
    .digest("hex");
}

function hashPasswordResetCode(emailCanonical: string, code: string) {
  return createHash("sha256")
...
async function deliverPasswordResetCode(user: {
  email: string;
  username: string;
}, code: string) {
...
router.post("/register", async (req, res) => {
  try {
    ...
    if (
      ...
      res.status(400).json({
        message:
          "Для регистрации нужно принять пользовательское соглашение, ознакомиться с политикой обработки персональных данных и дать согласие на обработку персональных данных",
      });
      return;
    }
...
router.post("/verify-email", requireAuth, async (req, res) => {
  try {
    ...
    if (
      hashVerificationCode(user.emailCanonical, code.trim()) !==
      user.emailVerificationCodeHash
    ) {
...
router.post("/resend-verification", requireAuth, async (req, res) => {
  try {
    ...
    const updatedUser = await prisma.user.update({
      ...
      data: {
        emailVerificationCodeHash: hashVerificationCode(
          user.emailCanonical,
          code
        ),
        emailVerificationExpiresAt: getEmailCodeExpiration(),
    ...
    try {
      ...
      res.status(502).json({
        message:
          "Не удалось отправить письмо. Проверь настройки EMAIL_PROVIDER / RESEND_API_KEY / MAIL_FROM и попробуй ещё раз.",
      });
    }
  } catch (error) {
    console.error(error);
...
router.post("/request-password-reset", async (req, res) => {
  try {
    ...
    const genericMessage =
      "Если аккаунт с таким email существует, код восстановления будет отправлен.";
...
router.post("/verify-password-reset-code", async (req, res) => {
  try {
    ...
    if (
      hashPasswordResetCode(user.emailCanonical, code.trim()) !==
      user.passwordResetCodeHash
    ) {
      ...
      const attemptsLeft = Math.max(
        PASSWORD_RESET_CODE_MAX_ATTEMPTS - nextAttemptCount,
        0
      );
