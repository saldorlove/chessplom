...
import { OPENING_QUALITY_SEEDS_V3 } from "../data/openingQualitySeedDataV3.js";
async function seedOpeningQualityV3() {
    let openingsTouched = 0;
    let variationsRecreated = 0;
    let stepsCreated = 0;
    for (const openingSeed of OPENING_QUALITY_SEEDS_V3) {
...
seedOpeningQualityV3()
    .catch((error) => {
    console.error("Не удалось применить v3 улучшения базы дебютов", error);
    process.exitCode = 1;
})
    .finally(async () => {
    await prisma.$disconnect();
});
//# sourceMappingURL=seedOpeningQualityV3.js.map