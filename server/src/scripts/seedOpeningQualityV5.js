...
import { OPENING_QUALITY_SEEDS_V5 } from "../data/openingQualitySeedDataV5.js";
async function seedOpeningQualityV5() {
    let openingsTouched = 0;
    let variationsRecreated = 0;
    let stepsCreated = 0;
    for (const openingSeed of OPENING_QUALITY_SEEDS_V5) {
...
seedOpeningQualityV5()
    .catch((error) => {
    console.error("Не удалось применить v5 улучшения базы дебютов", error);
    process.exitCode = 1;
})
    .finally(async () => {
    await prisma.$disconnect();
});
//# sourceMappingURL=seedOpeningQualityV5.js.map