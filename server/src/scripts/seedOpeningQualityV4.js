...
import { OPENING_QUALITY_SEEDS_V4 } from "../data/openingQualitySeedDataV4.js";
async function seedOpeningQualityV4() {
    let openingsTouched = 0;
    let variationsRecreated = 0;
    let stepsCreated = 0;
    for (const openingSeed of OPENING_QUALITY_SEEDS_V4) {
...
seedOpeningQualityV4()
    .catch((error) => {
    console.error("Не удалось применить v4 улучшения базы дебютов", error);
    process.exitCode = 1;
})
    .finally(async () => {
    await prisma.$disconnect();
});
//# sourceMappingURL=seedOpeningQualityV4.js.map