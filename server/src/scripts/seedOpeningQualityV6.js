...
import { OPENING_QUALITY_SEEDS_V6 } from "../data/openingQualitySeedDataV6.js";
async function seedOpeningQualityV6() {
    let openingsTouched = 0;
    let variationsRecreated = 0;
    let stepsCreated = 0;
    for (const openingSeed of OPENING_QUALITY_SEEDS_V6) {
...
seedOpeningQualityV6()
    .catch((error) => {
    console.error("Не удалось применить v6 улучшения базы дебютов", error);
    process.exitCode = 1;
})
    .finally(async () => {
    await prisma.$disconnect();
});
//# sourceMappingURL=seedOpeningQualityV6.js.map